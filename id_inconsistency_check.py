...
def compare_redcap_maganamed_ids(redcap_df, maganamed_df, save_path):
    ...
    # Extract ID sets
    redcap_df = pd.read_excel(redcap_df)
    redcap_ids = set(redcap_df['study_id_pat'])
    ...
    print('Missing IDs in maganamed', missing_in_maganamed)
    if missing_in_maganamed:
        df_missing = pd.DataFrame({'participant_identifier': list(missing_in_maganamed)})
        df_missing['note'] = 'Present in REDCap, missing in maganamed.'
    ...
    if missing_in_redcap:
        df_missing = pd.DataFrame({'participant_identifier': list(missing_in_redcap)})
        df_missing['note'] = 'Present in maganamed, missing in REDCap'
...
def compare_redcap_dmmh_ids(redcap_df, dmmh_df, save_path):
    ...
    redcap_df = pd.read_excel(redcap_df)
    redcap_ids = set(redcap_df['study_id_pat'])
    dmmh_ids = set(dmmh_df['participant_identifier'])

    # # IDs present in REDCap but missing in dmmh
    missing_in_dmmh = redcap_ids - dmmh_ids
    if missing_in_dmmh:
        df_missing_dmmh = pd.DataFrame({'participant_identifier': list(missing_in_dmmh)})
    ...
    if missing_in_redcap:
        df_missing_redcap = pd.DataFrame({'participant_identifier': list(missing_in_redcap)})
...
def compare_redcap_movisensESM_ids(redcap_df, movisensESM_df, save_path):
    ...
    redcap_df = pd.read_excel(redcap_df)
    redcap_ids = set(redcap_df['study_id_pat'])
    movisensESM_ids = set(movisensESM_df['participant_identifier'])
    ...
    if missing_in_movisensESM:
        df_missing_movisensESM = pd.DataFrame({'participant_identifier': list(missing_in_movisensESM)})
...
def compare_redcap_movisensSensing_ids(redcap_df, movisSensing_df, save_path):
    ...
    redcap_df = pd.read_excel(redcap_df)
    redcap_ids = set(redcap_df['study_id_pat'])
    movisensSensing_ids = set(movisSensing_df['participant_identifier'])
    ...
    if missing_in_movisensSensing:
        df_missing_movisensSensing = pd.DataFrame({'participant_identifier': list(missing_in_movisensSensing)})
